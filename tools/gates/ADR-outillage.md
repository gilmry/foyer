...
| Refactor Symfony / Doctrine (évolutions de stack) | **rector/rector** — rulesets intégrés `SymfonySetList` · `DoctrineSetList` (un seul paquet, MIT) | MIT | jalon | Les **indices de dépréciation des frameworks** (`trigger_deprecation` Symfony, `Doctrine\Deprecations\Deprecation`) sont la **source de vérité** du saut de version : on refactorise **leurs signaux**, pas les notes de version. Le ruleset du framework (intégrés au paquet Rector, pas des paquets séparés) fait la mécanique ; le compteur de dépréciations est un rapport de jalon (tendance décroissante), `migration-projet-existant.md` « Les évolutions de la stack de destination ». |
| Refactor ApiPlatform (annotations → attributs, patterns) | `invenso/rector` (tiers) | MIT | jalon | **Pas de ruleset officiel ApiPlatform** dans Rector : le tiers `invenso/rector` couvre annotations → attributs. Pour le reste, les dépréciations émises par `api-platform/core` (`trigger_deprecation`) sont traitées **à la main**, revue en jalon. |
